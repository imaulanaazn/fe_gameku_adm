...
const page = async ({ params }: IParams) => {
    ...
    const gameDetail = await sendRequest<IGameDetail>("/v1/game-detail?slug=" + params.productKey);
    if (!gameDetail.ok) {
        return <NotFound />;
    }
...
export const generateMetadata = async ({ params }: IParams) => {
    const meta = await sendRequest<IMeta>("/v1/meta?path=/" + params.productKey, {}, 3600);
