...
import { IImageCarousel, IImageCarouselPagination } from "@/interfaces/carousels";
...
const TableBanner: React.FC<{ banner: IImageCarouselPagination }> = ({ banner }) => {
    ...
    const getBanners = async () => {
        ...
        const req = await fetch(process.env.NEXT_PUBLIC_BASE_URL + "/v1/banner?" + searchParams.toString(), {
            cache: "no-cache",
            method: "GET",
            credentials: "include",
            headers: {
    ...
    return (
        <>
            ...
            {showDelete && <ConfirmDelete path="/v1/banner" method="DELETE" getNewData={getBanners} />}
            {loading ? (
                <Loading />
            ) : (
