...
const TableCategoryProduct: React.FC<{ productCategory: IProductCategoryPagination }> = ({ productCategory }) => {
    ...
    const [productCategories, setProductCategories] = useRecoilState(productCategoryAton);
    ...
    const getProductCategories = async () => {
        ...
        const req = await fetch(process.env.NEXT_PUBLIC_BASE_URL + "/v1/product-category?" + searchParams.toString(), {
            cache: "no-cache",
            method: "GET",
            credentials: "include",
            headers: {
    ...
    const handleSelectAll = () => {
        ...
        if (!selectAll) {
            setSelected(productCategories.data.map((productCategory) => productCategory.id));
        } else {
            setSelected([]);
        }
    };
                                                    ...
                                                    <td className="py-3 pl-4">
                                                        <div className="flex items-center h-5">
                                                            <input
                                                                ...
                                                                onChange={() => handleRowSelect(productCategory.id)}
