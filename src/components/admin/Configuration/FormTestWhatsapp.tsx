...
const FormTestWhatsapp: React.FC<IFormTestWhatsapp> = ({ handleShowForm }) => {
  const [template, setTemplate] = useState<{ label: string; value: string }[]>(
    []
  );
  ...
  const postTestWhatsapp = async () => {
    const toastId = toast.loading("Sedang mengirim whatsapp untuk test");
    const searchParams = new URLSearchParams();
    searchParams.append("mobileNumber", data.mobileNumber);
    searchParams.append("messageId", data.messageId);
  ...
  useEffect(() => {
    const newData = listTemplate.map((data) => {
      return {
        label: data.title,
        value: data.id,
      };
    ...
    <div className="absolute top-0 left-0 right-0 h-screen bg-black bg-opacity-20 flex items-center justify-center z-10">
      <div className="p-4 bg-white rounded-md w-1/3">
        <form onSubmit={handleTestWhatsapp}>
          <h1 className="text-lg font-semibold pb-4 border-b-2">
            Test Whatsapp
          </h1>
          ...
          <div className="mt-4 flex flex-col">
            <label htmlFor="messageId">Pilih Contoh Pesan</label>
            <Select
              id="messageId"
          ...
          <div className="flex items-center justify-end space-x-3 mt-4">
            ...
            <button
              disabled={!data.messageId && !data.mobileNumber}
              type="submit"
