...
const TableYoutubeVideo: React.FC<{ data: INewsVideosPagination }> = ({ data }) => {
    ...
    const getNewData = async (pagination?: Partial<IPagination>) => {
        ...
        const req = await fetch(process.env.NEXT_PUBLIC_BASE_URL + "/v1/youtube?" + searchParams.toString(), {
            cache: "no-cache",
            method: "GET",
            credentials: "include",
            headers: {
    ...
    return (
        <>
            ...
            {showDelete && <ConfirmDelete path="/v1/youtube" method="DELETE" getNewData={getNewData} />}
            {loading ? (
                <Loading />
            ) : (
                    ...
                    <div className={`p-5 ${selected.length > 0 ? "bg-green-200" : "bg-white"}`}>
                        {selected.length === 0 && (
                            <div className="flex items-center justify-between">
                                <p className="text-xl font-semibold">Youtube Video</p>
                                <div
                                    onClick={() => {
                                        ...
                                        <tbody className="divide-y divide-gray-200">
                                            {newData.data.map((data) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-6 py-4 text-sm text-gray-800">{data.url}</td>
