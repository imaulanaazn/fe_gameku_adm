...
const SearchResultModal = ({
    ...
    useEffect(() => {
        const getPopularGames = async () => {
            const request = await fetch(process.env.NEXT_PUBLIC_BASE_URL + "/v1/games?isPopular=true", {
                method: "GET",
                credentials: "include",
                cache: "no-cache",
                headers: {
        ...
        const getSearchedGames = async () => {
            const request = await fetch(process.env.NEXT_PUBLIC_BASE_URL + `/v1/games?search=${searchKeyword}`, {
                method: "GET",
                credentials: "include",
                cache: "no-cache",
                headers: {
    ...
    return (
        <div
            className={`${!showSearchModal && "hidden"} fixed top-0 left-0 w-full h-screen z-20 bg-[rgba(0,0,0,0.6)]`}
            ...
            >
                ...
                <div className="result p-4 lg:p-8 h-96 md:h-[50rem] lg:h-[30rem]  overflow-y-scroll">
                    {searchKeyword ? (
                        ...
                        <h1 className="font-semibold text-xl md:text-2xl text-neutral-900">Popular Games</h1>
                    )}
                    <h1></h1>
