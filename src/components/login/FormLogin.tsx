...
const FormLogin = () => {
    ...
    const handleSubmit = async (e: any) => {
        ...
        if (allowed) {
            ...
            const responseLogin = await fetch(process.env.NEXT_PUBLIC_BASE_URL + "/v1/customer/login", {
                method: "POST",
                credentials: "include",
                headers: {
    ...
    const getMe = async () => {
        const responseCustomer = await fetch(process.env.NEXT_PUBLIC_BASE_URL + "/v1/me", {
            method: "GET",
            credentials: "include",
            headers: {
                "content-type": "application/json",
    ...
    const handleRequestOTP = () => {
        if (!data.username) {
            return toast.warn("Silahkan isi terlebih dahulu nomor whatsapp atau email", {
                isLoading: false,
                position: "top-right",
                autoClose: 3000,
            });
    ...
    useEffect(() => {
        ...
        setDisableBtnReqOtp(!phoneRegex.test(data.username) && !emailRegex.test(data.username));
    }, [data.username]);
