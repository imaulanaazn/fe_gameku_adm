...
const ActionPayment: React.FC<IActionPaymentProps> = ({ invoice }) => {
    ...
    const handleClickCopy = () => {
        navigator.clipboard.writeText(
            invoice.category === "3" ? invoice.payment.accountNumber : invoice.payment.paymentCode,
        );
        setCopied(true);
        ...
        if (invoice.category === "2") {
            ...
            if (invoice.cd === "ID_OVO") {
                return (
                    ...
                    >
                        ...
                        <p>Silahkan cek notifikasi yang masuk ke aplikasi OVOmu untuk melanjutkan pembayaran!</p>
                    </div>
                );
                ...
                return (
                    ...
                    >
                        <FontAwesomeIcon icon={faInfoCircle} />
                        <p>Silahkan cek notifikasi yang masuk ke aplikasi JENIUSmu untuk melanjutkan pembayaran!</p>
                    </div>
                );
            } else {
                return (
            ...
            return (
                <div
                    className={`bg-blue-200 text-blue-800 p-4 h-fit w-full rounded-lg flex gap-3 items-center text-sm mt-3`}
                >
                    {invoice.category === "3" ? <p>Nomor Virtual Account : </p> : <p>Kode Pembayaran : </p>}
                    {invoice.category === "3" ? <p>{invoice.payment.accountNumber}</p> : invoice.payment.paymentCode}
