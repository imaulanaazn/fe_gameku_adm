import { Card, CardContent, Grid, CardHeader, Box, Avatar, Typography, Divider, Button, Alert } from "@mui/material";
...
const getStatusPayment = (status: OrderStatuses, type: OrderType, expiredAt: string) => {
    let msg;
    let msgBox;
    let severity;
    switch (status) {
        case OrderStatuses.PENDING_PAYMENT:
            msg = "Silahkan lakukan pembayaran sebelum agar orderanmu segera diproses";
            msg += `. Pembayaranmu akan expired pada ${dayjs(expiredAt).format("DD MMMM YYYY HH:mm:ss")}`;
            msgBox = "Menunggu Pembayaran";
            severity = "info";
            break;
        case OrderStatuses.EXPIRED:
            msg = "Pesananmu sudah expired pada " + dayjs(expiredAt).format("DD MMMM YYYY HH:mm:ss");
            severity = "error";
            msgBox = "Kadaluarsa";
            break;
        case OrderStatuses.PENDING_ORDER:
        case OrderStatuses.PROCESSING:
            msg = "Pesananmu sedang kami proses secepatnya. Terima kasih sudah menunggu";
            severity = "info";
            msgBox = "Sedang Diproses";
            break;
        case OrderStatuses.SUCCESS:
            msg = "Pesananmu sudah selesai, terima kasih sudah order denom di GASSKEUN TOPUP";
            severity = "success";
            msgBox = "Berhasil";
            break;
        default:
            msg = "Pesananmu gagal diproses. Jika sudah dibayar silahkan hubungi admin";
            severity = "error";
            msgBox = "Gagal";
            break;
    }
...
const NewPayment = ({ invoices }: { invoices: IInvoice }) => {
    ...
    const getOrder = async (fromInterval: boolean) => {
        const req = await fetch(process.env.NEXT_PUBLIC_BASE_URL + "/v1/order-detail/" + invoices.invoiceId, {
            headers: {
                "ngrok-skip-browser-warning": "true",
            },
            credentials: "include",
                            ...
                            <CardContent>
                                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
                                        <Avatar src={order.logoGame} variant="rounded" sx={{ width: 50, height: 50 }} />
                                        <Box
                                            sx={{ display: "flex", flexDirection: "column", justifyContent: "center" }}
                                        >
                                            <Typography
                                                variant="body2"
                                                sx={{ letterSpacing: "0.25px", fontWeight: 600, marginTop: 1.5 }}
                                            >
                                                {order.productName}
                                            </Typography>
                                            <Typography
                                    ...
                                    <Box>
                                        ...
                                        <Typography variant="body2" sx={{ fontWeight: 600, marginTop: 1.5 }}>
                                            {currencyConverter(order.totalAmt)}
                                        </Typography>
                                    </Box>
                                </Box>
                                <Box sx={{ marginTop: 4 }}>
                                    {(order.detail?.userId || order.detail?.serverId || order.detail?.username) && (
                                ...
                                <Grid container spacing={6}>
                                    <Grid item xs={6} md={2}>
                                        <Box sx={{ display: "flex", flexDirection: "column" }}>
                                            {order.detail?.userId && (
                                                <Typography variant="body2" sx={{ fontWeight: 600, marginTop: 2 }}>
                                                    User ID
                                                </Typography>
                                            )}
                        ...
                        <Card sx={{ position: "relative", marginTop: 4 }}>
                            ...
                            <CardContent>
                                ...
                                >
                                    ...
                                    <Typography variant="body2">{order.detail?.quantity}</Typography>
                                </Box>
                                <Box
                                    sx={{
                                        display: "flex",
                                ...
                                >
                                    ...
                                    <Typography variant="body2">
                                        {order.detail?.amount && order.detail?.quantity
                                            ? currencyConverter(order.detail.amount * order.detail.quantity)
                                            : "N/A"}
                                    </Typography>
                                </Box>
                                <Box
                                ...
                                >
                                    ...
                                    <Typography variant="body2">{currencyConverter(order.feeAmt)}</Typography>
                                </Box>
                                <Box
                                    sx={{
                                        display: "flex",
                                ...
                                >
                                    ...
                                    <Typography variant="body2">{currencyConverter(order.discAmt)}</Typography>
                                </Box>
                                <Divider />
                                <Box
                                    sx={{
                            ...
                            <CardContent>
                                {(order.cd === "ID_JENIUSPAY" || order.cd === "ID_OVO") &&
                                    order.status === OrderStatuses.PENDING_PAYMENT && (
                                        <Box sx={{ marginBottom: 4 }}>
                                            <Alert severity="info">
                                                Silahkan cek aplikasi {order.cd === "ID_OVO" ? "OVO" : "JENIUS"} mu
                                ...
                                <Card sx={{ marginTop: 4 }}>
                                    <CardContent>
                                        <Box>
                                            ...
                                            >
                                                <Typography>{getTitlePayment(order.category)}</Typography>
                                                <Box>
                                                    <Avatar
                                            ...
                                            {(order.category === PaymentsCategory.QRIS ||
                                                ...
                                                <Box sx={{ marginTop: 4 }}>
                                                    ...
                                                    >
                                                        <Canvas
                                                            text={
                                                                order.payment.qrString || order.payment.qrCheckoutString
                                                            }
                                                            options={{
                                            ...
                                            {(order.category === PaymentsCategory.RETAIL ||
                                                order.category === PaymentsCategory.VIRTUAL_ACCOUNT) && (
