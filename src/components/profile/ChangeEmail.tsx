...
const ChangeEmail = () => {
    ...
    const handleSubmitChangeEmail = async (e: FormEvent<HTMLFormElement>) => {
        ...
        const req = await fetch(process.env.NEXT_PUBLIC_BASE_URL + "/v1/customer/update?type=email", {
            method: "PUT",
            credentials: "include",
            headers: {
    ...
    return (
        <>
            <div className="flex flex-col items-center gap-3 my-10">
                <h1 className="font-pulse font-semibold text-2xl">Ganti Email</h1>
            </div>
            <form onSubmit={handleSubmitChangeEmail} className="w-full max-w-md text-xs text-black">
                <div className="mb-4">
                    <input
                        ...
                        value={data.oldEmail}
                        onChange={(e) => setData((prev) => ({ ...prev, oldEmail: e.target.value }))}
                        required
                ...
                <div className="mb-4">
                    <input
                        ...
                        className="w-full p-4 border"
                        value={data.newEmail}
                        onChange={(e) => setData((prev) => ({ ...prev, newEmail: e.target.value }))}
                        required
